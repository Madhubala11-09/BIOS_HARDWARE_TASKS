...
# LOW PASS FILTER:
In order to filter one particular radio station,it also prevents aliasing (When high-frequency components fold-back into low frequency components) we use this block.
This block removes the unwanted high frequency, and limit the bandwidth. Since each fm radio station is 200KHz wide, that means 100KHz on each side, hence we set the cutoff frequency to 100.
...
We change the decimation to 5 in order to filter the central frequency, decimation is used for Downsampling, where the 5 th sample of signal is kept and rest is discarded. Sampling is the process of converting analog to digital signal. Change the bandwidth in frequency sink also by 5. 
